...
{
	[SerializeField] string		_inputPrefix;
	[SerializeField] Transform	_toe;
	[SerializeField] Transform	_hip;
	[SerializeField] Transform	_vesselRoot;
	[SerializeField] Transform	_oar;
	[SerializeField] Transform	_oarAnchorPoint;
	[SerializeField] Vector3	_oarLookAtUpVector;
	...
	void Update()
	{
		Vector2 analogStick = new Vector2(Input.GetAxis(_inputPrefix + "_X"), Input.GetAxis(_inputPrefix + "_Y"));
		float angle = AxisAngle(_inputPrefix == "Left" ? true : false, analogStick);
